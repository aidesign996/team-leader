...
上图说明调节回路；[完整文章中的增强回路图](https://aidesign996.github.io/ai-practice/article-team-leader.zh.html#figure-2)说明经验怎样反馈到提炼与验证。这是工作方法的设计思路，尚未建立可计算的系统动力学模型，也不表示 AI 能力会自动持续增长。
...
“让读者容易懂”是目标，“分段、加粗”是某次的办法。下次满页都在加粗，负责人可能反而减少强调。同一目标，可以导向不同做法。这个例子用于解释判断方法，并非效果实验。

规则与职责放在 `AGENTS.md`、`roles/` 等已有入口；案例通过 `docs/METHODOLOGY.md` 等经验入口查找，保留当时的条件、理由和实际结果。做完后，问题仍在就继续修正当前成果；有新收获再更新原经验。

**目标与有效要求必须遵守；旧案例可以调整。** 保存文件之后，还要在工作中实际检索和应用。这不等于重新训练模型，也不保证长期自动无遗漏。
